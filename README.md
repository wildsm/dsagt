...
AIDRIN as the check around every tabular pipeline stage. `aidrin` is a registered code in every project, and with the check on `dsagt-run` notes, after each pipeline step, every table the step read or wrote that has no report for its current content, so the agent checks it with the `aidrin` skill; `readiness_reports` returns a file's current report on request.
